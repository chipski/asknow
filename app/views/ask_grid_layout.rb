class AskGridLayout < UICollectionViewLayout
  ...
  def commit_init
    @column_count  ||= 2
    @item_width    ||= (UIScreen.mainScreen.bounds.size.width / @column_count) - 4 # 140.0
  ...
  def section_inset=(inset)
    ...
    NSLog("Grid.layout @section_inset.left=#{@section_inset.left} @section_inset.right=#{@section_inset.right}")
  end
  
  def dealloc
    NSLog("AskGridLayout dealloc")
    @column_heights = nil
    @item_attrs = nil
  end
  ...
  def prepareLayout
    ...
    NSLog("AskGridLayout this should never happens.") if @column_count < 1
